...
/// BRIEF §5.3's five buckets plus Subcontractors (DECISIONS 60); DECISIONS 2, 25, 32.
final class Probe_spec_Bucket: XCTestCase {
    func testSixCasesInDisplayOrder() {
        XCTAssertEqual(Bucket.allCases.count, 6)
        XCTAssertEqual(Bucket.allCases, [.labor, .equipment, .materials, .consumables, .subcontractors, .overhead])
    }

    func testCodableRawValuesAreLowercaseNames() throws {
        XCTAssertEqual(Bucket.allCases.map(\.rawValue), ["labor", "equipment", "materials", "consumables", "subcontractors", "overhead"])
        let json = try JSONEncoder().encode(Bucket.allCases)
        XCTAssertEqual(String(decoding: json, as: UTF8.self), #"["labor","equipment","materials","consumables","subcontractors","overhead"]"#)
    ...
    func testRowKindMapping() {
        ...
        XCTAssertEqual(Bucket.subcontractors.rowKind, .quantity)
    }

    func testFixedUnitsAndAnnualFlag() {
        // DECISIONS 25: hourly buckets fixed ("hr", "hr", "yr"); quantity buckets free text.
        XCTAssertEqual(Bucket.allCases.map(\.fixedUnit), ["hr", "hr", nil, nil, nil, "yr"])
        ...
        XCTAssertEqual(Bucket.allCases.map(\.title), ["Labor", "Equipment", "Materials", "Consumables", "Subcontractors", "Overhead"])
    }
}
